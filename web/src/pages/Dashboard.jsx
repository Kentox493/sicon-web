...
import { Play, Activity, Globe, Shield, Search, Server, FileCode, Layers } from 'lucide-react';
...
const Dashboard = () => {
    ...
    const handleStartScan = () => {
        ...
        console.log('Starting scan for:', targetUrl, scanOptions);
        navigate('/scan');
    };

    const stats = [
        { label: 'Total Scans', value: '128', icon: Activity, color: 'text-blue-400' },
        { label: 'Vulnerabilities', value: '3,402', icon: Shield, color: 'text-red-400' },
        { label: 'Domains', value: '542', icon: Globe, color: 'text-green-400' },
    ];
                ...
                <CardContent className="space-y-6">
                    <div className="flex flex-col md:flex-row gap-4">
                        <div className="flex-1">
                            <label className="text-sm font-medium text-text-secondary mb-2 block">Target URL / IP</label>
                            <Input
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <ScanOption
                                label="WAF Detection"
                                active={scanOptions.waf}
                                onClick={() => toggleOption('waf')}
                                icon={Shield}
                            />
                            <ScanOption
                                label="Port Scan"
                                active={scanOptions.port}
                                onClick={() => toggleOption('port')}
                                icon={Server}
                            />
                            <ScanOption
                                label="Subdomains"
                                active={scanOptions.subdo}
                                onClick={() => toggleOption('subdo')}
                                icon={Globe}
                            />
                            <ScanOption
                                label="CMS Detect"
                                active={scanOptions.cms}
                                onClick={() => toggleOption('cms')}
                                icon={Layers}
                            />
                            <ScanOption
                                label="Tech Stack"
                                active={scanOptions.tech}
                                onClick={() => toggleOption('tech')}
                                icon={FileCode}
                            />
                            <ScanOption
                                label="Directories"
                                active={scanOptions.dir}
                                onClick={() => toggleOption('dir')}
                                icon={Search}
                            />
                            <ScanOption
                                label="WordPress Enum"
                                active={scanOptions.wp}
                                onClick={() => toggleOption('wp')}
                                icon={FileCode}
...
const ScanOption = ({ label, active, onClick, icon: Icon, className }) => (
    <button
      ...
      ${active
                ? 'bg-accent-primary/10 border-accent-primary text-text-primary shadow-[0_0_10px_rgba(0,217,255,0.1)]'
